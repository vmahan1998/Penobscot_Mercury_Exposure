...
This section documents the logic and equations underlying seaward (downstream) migration. The modeled behavior accounts for the physical challenge of swimming against riverine flows and with tidal flows by incorporating agent-level state variables such as difficulty, energy, and stress.
...
$$D_{f} = 1 + 9 \cdot \left(\frac{V_{patch} - V_{min}}{V_{max} - V_{min}}\right)$$
...
$$ D_{f} = \left( \frac{|V_{patch}|}{V_{max} \cdot \left(\frac{M_{agent}}{M_{max}}\right)} \right)^k $$
...
-   $V_{patch}$ is the depth-averaged water velocity for the agent's current patch.
-   $D_{f}$ is the swimming difficulty factor.
-   $k$ is the scaling exponent for difficulty
